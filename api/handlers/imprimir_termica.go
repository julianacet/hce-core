...
func enviarImpresora(printerName, filePath string) error {
	...
	if os.Getenv("PRINTER_MODE") == "http" {
		return enviarHTTP(printerName, filePath)
	}
...
// enviarHTTP reenvía los bytes ESC/POS al relay de impresión que corre en el
// host Windows (accesible desde el container vía host.docker.internal).
func enviarHTTP(printerName, filePath string) error {
