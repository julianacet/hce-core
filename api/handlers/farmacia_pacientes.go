...
type farmaciaPacientesHandler struct{ db *pgxpool.Pool }

func FarmaciaPacientesRouter(db *pgxpool.Pool) http.Handler {
	h := &farmaciaPacientesHandler{db: db}
	r := chi.NewRouter()
	r.Get("/", h.buscar)
...
// GET /api/farmacia/pacientes?q=texto — busca pacientes para farmacia (solo lectura)
func (h *farmaciaPacientesHandler) buscar(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Query(r.Context(), `
		...
		LIMIT 20`, param)
	if err != nil {
		log.Printf("farmacia buscar pacientes: %v", err)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.Documento, &p.TipoDocumento, &p.NombreCompleto, &p.Telefono, &p.Email); err != nil {
			log.Printf("farmacia escanear paciente: %v", err)
			continue
		}
...
// GET /api/farmacia/pacientes/:documento
func (h *farmaciaPacientesHandler) obtener(w http.ResponseWriter, r *http.Request) {
